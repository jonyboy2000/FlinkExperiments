...
class TrackingPacketSource extends SourceFunction[TrackingPacket] {
  ...
  override def run(ctx: SourceContext[TrackingPacket]): Unit = {
    ...
    breakable {
      for ((millis, lat, lon) <- TrackingSample.data)
      {
        if (cancelled) { break }
        Console.println(s"Sleeping $millis")

        Thread.sleep(millis)
        val tp = TrackingPacket(ccn = ccn, tripid = tripid, millisecondsSinceEpoch = System.currentTimeMillis, lat = lat, lon = lon)
        ctx.collect(tp)
      }
    }
  }
